...
>
> - ReadinessProbe: 트래픽을 받아도 되는가를 판단
>   - 실패시 컨테이너는 재시작되지 않으며 대신 Service, Endpoint, LoadBalancer 에서 트래픽 대상에 제외가 된다
>   - OS 가 초기 실행될때 부팅 시간을 확보 → 살아는 있지만 준비 안된 상태를 판단할 수 있음
> - LivenessProbe: 해당 컨테이너가 죽었는지를 판단
>   - 실패시 kubelet 이 컨테이너를 강제로 재시작
>   - 프로세스는 살아있으나 데드락, 무한루프, 내부 스레드 정지등의 이슈로 외부 요청에 대한 응답을 하지 못함 → 이 경우 Kubernetes 입장에서는 죽은 것이나 마찬가지 이므로 외부에서 상태를 찔러보고 안된다면 재시작하는 개념
>
> 3 가지 공식적인 체크 방식
>
> - exec 방식
>   - `/tmp/ready.txt` 를 `readinessProbe.exec.command` 로 설정하면 kubelet 이 컨테이너 내부에서 해당 명령을 실행하여 exit code 가 0 이면 Ready 로 전환
> - httpGet 방식 (가장 흔한 방식)
>   - `readinessProbe.httpGet.path` / `port` 를 구성하여 해당 path 에 kubelet 이 컨테이너 IP 로 HTTP 요청을 수행하며 응답으로 200 ~ 399 상태코드가 온다면 Ready 로 전환
> - tcpSocket 방식
>   - `readinessProbe.tcpSocket.port` 를 설정하여 TCP 연결 가능 여부만 확인하며 애플리케이션 로직까지는 준비되었는지 알지못한다
>   - DB 혹은 단순 TCP 서버에서는 가끔 사용한다
